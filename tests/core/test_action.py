...
def test_requested_effect_accepts_different_domains() -> None:
    ...
    access = RequestedEffect(
        parameters={"role": "admin", "duration_minutes": 30}
    )
    database = RequestedEffect(
        parameters={"database": "customers", "environment": "production"}
    )
...
def test_requested_effect_cannot_be_modified() -> None:
    effect = RequestedEffect(parameters={"environment": "production"})
...
def test_requested_effect_rejects_non_json_values() -> None:
    with pytest.raises(TypeError):
        RequestedEffect(parameters={"invalid": object()})


def test_requested_effect_rejects_non_string_keys() -> None:
    with pytest.raises(TypeError):
        RequestedEffect(parameters={123: "invalid"})


def test_requested_effect_rejects_non_finite_numbers() -> None:
    with pytest.raises(ValueError):
        RequestedEffect(parameters={"amount": float("nan")})
...
def test_action_represents_generic_proposal() -> None:
    ...
    assert action.requested_effect.parameters["environment"] == "production"
...
def test_same_action_contract_supports_different_domains() -> None:
    ...
    database_action = Action(
        ...
        principal=principal,
        agent=agent,
        resource=Resource(resource_id="database-1"),
        requested_effect=RequestedEffect(
            parameters={"environment": "production"}
        ),
    )
    ...
    access_action = Action(
        ...
        principal=principal,
        agent=agent,
        resource=Resource(resource_id="account-1"),
        requested_effect=RequestedEffect(
            parameters={"role": "operator", "duration_minutes": 30}
        ),
    )
    ...
    payment_action = Action(
        ...
        principal=principal,
        agent=agent,
        resource=Resource(resource_id="payment-system-1"),
        requested_effect=RequestedEffect(
            parameters={"amount": 5000, "currency": "USD"}
        ),
    )
...
def test_action_requires_non_empty_identifiers(
    ...
    values = {
        ...
        "agent": Agent(agent_id="agent-456"),
        "resource": Resource(resource_id="database-789"),
        "requested_effect": RequestedEffect(parameters={}),
    }
